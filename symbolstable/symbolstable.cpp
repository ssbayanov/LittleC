...
arraytablerecord *SymbolsTable::insertArray(QString name, QString key, ValueTypeEnum type)
{
    arraytablerecord *array;
    try {
        array = new arraytablerecord(name, key, type, this);
    }
    catch (std::bad_alloc& ba) {
        return NULL;
    }
